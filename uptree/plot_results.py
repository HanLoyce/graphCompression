...
def main():
    ...
    base_csv = workspace_root / "result" / "benchmark_results.csv"
    sota_csv = workspace_root / "result" / "vssota.csv"

    if not base_csv.exists():
        print("Error: benchmark_results.csv not found. Run benchmark first.")
        return

    base_df = load_benchmark_csv(base_csv)
    if base_df.empty:
        print("Error: benchmark_results.csv has no usable records.")
        return
